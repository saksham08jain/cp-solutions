...
for _ in range(tcs):
    ...
    for i in range(n-2,-1,-1):
        # numbers at same index must be same 
        # or must be the xor of next number 
        # whats the identity of xor 
        # 0 right 0 xor 1=1 0 xor 0=0 
        if src[i]==dest[i]:
            
            continue
        elif src[i]^dest[i+1]==dest[i]:
           
            continue
        elif src[i]^src[i+1]==dest[i]:
