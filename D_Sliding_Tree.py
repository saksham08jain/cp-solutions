...
tcs=inp()
for _ in range(tcs):
    # I am totally not sure about this 
    # This is an arrow in the dark 
    # I think we want to push the neighbours down 
    # ie towards the leaves 
    # the oreder shouldnt matter in the sense taht 
    # every vertex with degree>=2 has to be operated on atleast o0nce 
    # ah but ig order matters cause where you put it matter 
    # cause it may take too long to go down towards the leaves? 
    # but in each subtree we need to make this sure separately right 
    # i can do this i can start from the leaves 
    # and start puhing the nieghbours down 
    # i dont have enough time to think left 
    # I will check this 
    # push any leaves adjacent 's neighbours towards the leaaf if it isnt 
    ...
    for i in range(1,n+1):
        if len(adj[i])!=1:
            continue
        #now only leaves will reach this part of the loop 
        # check the parent of the leaf 
        parent=adj[i][0]
        # the parent maybe linked to many subtrees 
        #  i need to keep one of em and send the rest to the leaf 
        # the question is which one will i keep? 
        # pick any of the parents neightbour except i 
        a=-1
        for j in adj[parent]:
            if j!=i:
                a=j 
