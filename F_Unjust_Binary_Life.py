...
***************************************************************************************
'''
#  need to review this in the evening 
#NO DO THIS RIGHT NOW 
ONLINE_JUDGE = False
set_online_judge()
tcs=inp()
for _ in range(tcs):
    ...
    a=st() 
    b=st()
    # xor alone isnt enough cause what if we had a dependency flip 
    # what if the column or the row was already flipped in the path??
    # 
    last_row=[float('inf')]*(n+1)
    last_row[1]=0
    for i in range(n):
        cur_row=[float('inf')]*(n+1)
        
        for j in range(1,n+1):
            xorr=int(a[i])^int(b[j-1])
            
            cur_row[j]=min(cur_row[j-1],last_row[j])#min f(x-1,y),f(x,y-1)
            if xorr==1:
                cur_row[j]+=1 # actually what did i flip doesnt even matter? damnn 
            ans+=cur_row[j]
        print(f'{i}th row',cur_row)
        last_row=cur_row
    print(ans)