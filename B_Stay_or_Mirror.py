...
for _ in range(tcs):
    ...
    p=lis() 
    cur_inversions=0 
    idx=[0]*(n+1) #idx of the number
    for i in range(n):
        idx[p[i]]=i
        for j in range(i+1,n):
            if p[i]>p[j]:
                cur_inversions+=1 
    #actually counting conversions can be optimised in n log n 
    
    dInversions=0 #delta inversions if i can switch 
    for i in range(1,n+1):
        #start with i 
        left=idx[i]#elemnts to the left of i 
        right=n-idx[i]-1 #elements to the right of i 
        for j in range(1,i):
            if idx[j]>idx[i]:
                right-=1 
            else:
        ...
        if right-left<0:
            dInversions+=right-left 
    print(cur_inversions+dInversions)
