...
for _ in range(tcs):
   ...
   #first make all even numbers prefmaxx 
    maxx=-1
    ...
    for i in range(2,n+1,2):
        ...
        #but it is required to be greather than next number 
        #hence i need to decrease next number 
        if arr[i]==arr[i-1]:
            ops+=1 
        if i!=n:
            ops+=max(0,arr[i+1]-arr[i]+1)
            arr[i+1]=arr[i]-1

    print(ops)

    #done 
    #now check what needs to be decreased and by how much 
